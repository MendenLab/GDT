...
def parse_lab_data(text_data):
    ...
    Since llama hallucinates so much, we cannot use TwinWeaver and need to manually parse the text.
    ...
    # NEW: Pattern for lines like "3 chloride - 2075-0 106.4"
    week_first_space_pattern = re.compile(
        r"^\s*(\d+)\s+(?!weeks?)(.*?)\s*-\s*([\d-]+)\s+([\d.]+)\s*$",
        re.IGNORECASE
    )
    ...
    for line in lines:
        ...
        # 7. NEW: Check for "3 chloride - 2075-0 106.4" format
        match = week_first_space_pattern.match(line)
        if match:
            try:
                week = int(match.group(1))
                lab_name_part = match.group(2).strip()
        ...
        # 8. Check for "creatinine - 2160-0, week 4..." format
        ...
        # 9. Check for the most generic single-line format
