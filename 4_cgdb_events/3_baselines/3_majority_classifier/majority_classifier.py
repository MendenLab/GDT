...
def main(load_folder_splits_train, load_folder_splits_test):
    ...
    for indication in all_indications:
        ...
        all_targets = glob.glob(os.path.join(folder_of_labels, "*_targets.csv"))
        all_targets_dfs = [pd.read_csv(f) for f in all_targets]
        train_labels = pd.concat(all_targets_dfs, ignore_index=True)
    ...
    for variable in all_curr_variables:
        ...
        for timeline in all_timelines:
            ...
            #: get subset, only containing non censored samples (ie. really biased and naive approach)
            subset = all_train_labels[
                (all_train_labels["sampled_category"] == variable) & 
                (all_train_labels["week_to_predict"] == timeline) &
                (pd.isna(all_train_labels["censored"]))
            ].copy()

            #: get majority class for variable and timeline, but only for non censored
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train heads and evaluate for CLIMBR-T")
