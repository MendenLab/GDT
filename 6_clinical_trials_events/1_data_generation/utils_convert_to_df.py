...
class ConvertToDF:
    ...
    def _get_ml_input_df(self, data):
        ...
        curr_patientid = constant["patientid"].iloc[0].split("_var")[0]  # Extract the patientid up to the variable specification (since its agnostic)
        ...
        # Add meta
        new_constant["generic_patientid"] = curr_patientid
    ...
    def convert_split_data_to_input_and_output_df(self, data):
        ...
        # Get the ML standard input DF
        ml_input_df = self._get_ml_input_df(data)

        #: get full input DF
        full_input_df = data["events_until_split"].copy()
        # Extract the patientid up to the variable specification (since its agnostic)
        full_input_df["patientid"] = full_input_df["patientid"].iloc[0].split("_var")[0]  

        #: get constant DF
        constant_df = data["constant_data"]
        # Extract the patientid up to the variable specification (since its agnostic)
        constant_df["patientid"] = constant_df["patientid"].iloc[0].split("_var")[0]  
        ...
        return ml_input_df, full_input_df, constant_df, target_df
