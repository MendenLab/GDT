...
def eval(test_targets_path, test_text_path, prediction_url, model_name, tokenizer_path):
    ...
    # FIX: Use .apply() with axis=1 to operate on each row.
...
def main(port, test_targets_path, test_text_path, model_to_eval, wandb_group, skip_vllm_launch, tokenizer=TOKENIZER, python_path=PYTHON_PATH):
    ...
    wandb.run.name = f"Genie DT - Eval - Landmark - Probability"
    wandb.config.update({
        "indication": indication,
    ...
    if skip_vllm_launch:
        ...
        print("\nWaiting 10 minutes for the vLLM server to load...")
        time.sleep(10 * 60)
    
    print("\nStarting evaluation...")
    eval(test_targets_path, test_text_path, prediction_url, model_to_eval, tokenizer_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_targets_path", type=str, default="genie-dt-cit-eval-events/0_data/test/", help="Path to the test targets file for evaluation")
    parser.add_argument("--test_text_path", type=str, default="genie-dt-cit-eval-events/0_data/test_text/text_table_cit_test_num_samples_per_lot_1.csv", help="Path to the test text file for evaluation")
    ...
    parser.add_argument("--wandb_group", type=str, default=MODEL_NAME, help="W&B group")
    ...
    main(args.port,args.test_targets_path, args.test_text_path, args.model_to_eval, args.wandb_group, args.skip_vllm_launch, args.tokenizer, args.python_path)
