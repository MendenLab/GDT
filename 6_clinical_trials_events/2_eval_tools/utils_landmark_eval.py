...
CHECKSUM_PATH = "genie-dt-cit-eval-events/0_data/checksums/landmark/"

TRAIN_DATA_PATH = "genie-dt-cit-eval-events/0_data/train/"



class LandmarkEventsEval:
    ...
    def _load_data(self):
        ...
        self.target_df = pd.read_csv(os.path.join(self.data_loading_path, f"targets_{self.split}_{self.indication}.csv"))

        # Convert 'censored = False' to pd.NA, since that is 
        self.target_df.loc[self.target_df["censored"] == False, "censored"] = pd.NA
        ...
        # Get in trial id
        self.target_df["trial_id"] = self.target_df["patientid"].apply(lambda x: x.split("_")[0])
    ...
    def _get_probability_metrics(self, predictions_subset, target_subset, variable, week):
        ...
        NOTE: very inefficient, but works well for this use case.
        ...
        Returns:
            dict: A dictionary containing the calculated 'auc', 'brier_score', and 'c_index'.
        ...
        # Test data from the current evaluation subset
    ...
    def evaluate(self, predictions, log=True):
        ...
        all_results = {}
        all_trials = self.target_df["trial_id"].unique()
        predictions["trial_id"] = predictions["patientid"].apply(lambda x: x.split("_")[0])
        ...
        for event_type in list_of_event_types:
            ...
            for week in list_of_weeks:
                all_results[event_type][week] = {}
                                
                for trial_id in all_trials:

                    # Filter the target dataframe for the current event type and week
                    target_subset_all = self.target_df[(self.target_df["sampled_category"] == event_type) & (self.target_df["week_to_predict"] == week) & (self.target_df["trial_id"] == trial_id)]
                    
                    # Filter the predictions dataframe for the current event type and week
                    predictions_subset_all = predictions[(predictions["sampled_category"] == event_type) & (predictions["week_to_predict"] == week) & (predictions["trial_id"] == trial_id)].copy()
                    ...
                    if wandb.run is not None and log:
                        wandb.log({
                            f"{trial_id}_{event_type}_week_{week}_brier_score": metrics["brier_score"],
                            f"{trial_id}_{event_type}_week_{week}_c_index": metrics["c_index"],
                        })
                    
                    # Store the results in the all_results dictionary
                    all_results[event_type][week][trial_id] = metrics

        #: save predictions and targets DF to W&B
        if wandb.run is not None and log:
            predictions_df = predictions.copy()
            targets_df = self.target_df.copy()
            wandb_table_predictions = wandb.Table(dataframe=predictions_df)
            wandb_table_targets = wandb.Table(dataframe=targets_df)
            wandb.log({
