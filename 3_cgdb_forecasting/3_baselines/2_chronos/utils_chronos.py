...
TRAIN_DATA_STATS = "/0_data/train_data_stats/"
...
def apply_3_sigma_filtering_and_standardization(raw_input_events, indication, raw_targets=None, verbose=True, standardize=True, train_data_stats_path=TRAIN_DATA_STATS):
    ...
    variable_stats = pd.read_csv(train_data_stats_path + f"{indication}_train_data_stats.csv")
...
def finetune_chronos_model(chronos_model, train_ag_data, prediction_length, log_path,
                           ...
                           nr_validation_samples=500,):

    
    # In a few very rare cases, we have the same train patient in 2 different indications, which was 
    # actually happening in the raw data. 

    # 1. Clean the main DataFrame's (item_id, timestamp) index
    if train_ag_data.index.has_duplicates:
        print("Removing duplicates from main DataFrame's MultiIndex...")
        train_ag_data = train_ag_data[~train_ag_data.index.duplicated(keep='first')]

    # 2. Clean the static_features DataFrame's index (the new, crucial step)
    if train_ag_data.static_features is not None and train_ag_data.static_features.index.has_duplicates:
        print("CRITICAL: Removing duplicates from the static_features index...")
        
        # Get the static features DataFrame
        static_df = train_ag_data.static_features
        
        # Create a new DataFrame with the duplicates removed from its index
        cleaned_static_df = static_df[~static_df.index.duplicated(keep='first')]
        
        # Re-assign the cleaned static features back to the TSDataFrame. This is essential!
        train_ag_data.static_features = cleaned_static_df

    print("Data cleaning complete. Proceeding to create train/validation split.")
    ...
    # Note, we ensure that we slice entire patients based on the main script
    # Then use the predictor settings to ensure validation is done correctly.
    train_set = train_ag_data[:len(train_ag_data) - nr_validation_samples]
    validation_set = train_ag_data[len(train_ag_data) - nr_validation_samples:]
    ...
    predictor = predictor.fit(
        ...
        tuning_data=validation_set,
        num_val_windows=1,
    )

    # Hacky way to get the model path, which is what is actually needed for later predictions due to bug in AutoGluon
...
def predict_chronos_model_on_input(prediction_length, train_ag_data, chronos_model):
    ...
    if isinstance(chronos_model, str):
        ...
        predictor = TimeSeriesPredictor(
            ...
            freq="W-MON",
            target="target",
            eval_metric="MAE", # Mean Absolute Scaled Error, good for diverse series
            verbosity=4, # For debugging
        )

        # Fit the predictor (for zero-shot with Chronos, this loads the pre-trained model)
