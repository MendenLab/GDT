...
def main(split, base_path):
    ...
    for indication in all_indications:
        ...
        wandb.init(project="genie-dt-cgdb-baselines-forecasting", mode="offline" if DEBUG else "online",
                   group="chronos_bolt_zero_shot_fixed")

        # Setting up the model parameters
        prediction_length = 13
        chronos_model = "autogluon/chronos-bolt-base"

        wandb.run.name = f"Chronos Bolt - Zero Shot - {split} - {indication}"
        wandb.config.update({
            "split": split,
            "base_path": base_path,
            "indication": indication,
            "model": "chronos_bolt_zero_shot",
