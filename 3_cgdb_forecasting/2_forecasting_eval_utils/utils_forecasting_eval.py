...
CHECKSUM_PATH = "~/genie-dt-cgdb-baselines-forecasting/0_data/checksums/"
TRAIN_DATA_STATS = "~/genie-dt-cgdb-baselines-forecasting/0_data/train_data_stats/"
...
def apply_last_observed_value_to_target(input_df, empty_target_df):
    ...
    #: for each input, get last observed value, grouped by patientid and sorted by date
...
class ForecastingEval:
    ...
    def _load_data(self):
        constant_df_path = self.data_loading_path + f"constant_df_{self.indication}_{self.split}.csv"
        input_df_path = self.data_loading_path + f"input_df_{self.indication}_{self.split}.csv"
        target_df_path = self.data_loading_path + f"target_df_{self.indication}_{self.split}.csv"
        meta_data_path = self.data_loading_path + f"meta_data_{self.indication}_{self.split}.csv"
        ...
        self.variable_stats = pd.read_csv(TRAIN_DATA_STATS + f"{self.indication}_train_data_stats.csv")
    ...
    def _eval_metrics(self, predicted_targets, target_df, event_name):
    ...
    def _apply_3_sigma_capping(self, predictions, target_df=None, verbose=True):
        ...
        assert (predictions.loc[not_capped_predictions_mask, "event_value"] == original_predictions_event_value[not_capped_predictions_mask]).all(), \
    ...
    def evaluate(self, predicted_targets):
        ...
        assert [x in ["date", "event_value", "patientid", "event_name"] for x in predicted_targets.columns], "predicted_targets should have the columns ['date', 'event_value', 'patientid', 'event_name']"
        assert [x in ["date", "event_value", "patientid", "event_name"] for x in self.target_df.columns], "target_df should have the columns ['date', 'event_value', 'patientid', 'event_name']"
        ...
        if self.checksum_dict[self.data_loading_path][self.split][self.indication] == {}:
            ...
            assert self.checksum_dict[self.data_loading_path][self.split][self.indication]["number_of_patients"] == number_of_patients, f"Number of patients in predicted_targets ({number_of_patients}) does not match the number of patients in target_df ({self.checksum_dict[self.data_loading_path][self.split][self.indication]['number_of_patients']})"
            assert self.checksum_dict[self.data_loading_path][self.split][self.indication]["number_of_events"] == number_of_events, f"Number of events in predicted_targets ({number_of_events}) does not match the number of events in target_df ({self.checksum_dict[self.data_loading_path][self.split][self.indication]['number_of_events']})"
            ...
            assert np.array_equal(self.checksum_dict[self.data_loading_path][self.split][self.indication]["list_of_patient_ids"], list_of_patient_ids), f"List of patient ids in target_df does not match the list of patient ids in target_df"
        ...
        predicted_targets, target_df_capped = self._apply_3_sigma_capping(predicted_targets, self.target_df)
        ...
        for target_variable in all_unique_targets:
            ...
            final_results[target_variable]["metrics"] = self._eval_metrics(predicted_targets, target_df_capped, target_variable)
            final_results[target_variable]["target_variable"] = target_variable
            final_results[target_variable]["target_variable_name"] = target_df_capped[target_df_capped["event_name"] == target_variable]["event_descriptive_name"].dropna().values[0]
            final_results[target_variable]["num_samples"] = len(target_df_capped[target_df_capped["event_name"] == target_variable])
            final_results[target_variable]["num_patient_splits"] = len(self._get_unique_patient_samples(target_df_capped[target_df_capped["event_name"] == target_variable]["patientid"].unique()))
            final_results[target_variable]["num_patients"] = len(self._get_unique_raw_patientids(target_df_capped[target_df_capped["event_name"] == target_variable]["patientid"].unique()))
            final_results[target_variable]["num_unique_dates"] = len(target_df_capped[target_df_capped["event_name"] == target_variable]["date"].unique())
